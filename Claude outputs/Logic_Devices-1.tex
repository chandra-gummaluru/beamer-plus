...
\definecolor{segdim}{RGB}{120,26,20}     % on, but not the one being discussed
...
% When some segments are emphasised, the other lit ones are drawn dimmer, so
% the eye goes to the segment being discussed.
\newcommand{\sevenseg}[4]{%
  % The lists may arrive as macros (\segFive), so expand them before looping.
  \edef\seglit{#3}\edef\seghot{#4}%
  \ifx\seghot\empty \def\segon{seglit}\else \def\segon{segdim}\fi
  \begin{scope}[shift={#1}, scale=#2]
    ...
    \foreach \s/\o/\x/\y/\u/\v in \segcoords {
      ...
      \foreach \t in \seglit {%
        ...
        \ifx\sega\segb \segbar{\segon}{\o}{\x}{\y}{\u}{\v}\fi}%
      \foreach \t in \seghot {%
        \edef\sega{\s}\edef\segb{\t}%
